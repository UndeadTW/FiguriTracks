...
#include <stdlib.h>
#include <time.h> 
#include <windows.h> // LIBRERIA PARA LA ANIMACION 
...
// --- ESTRUCTURAS Y CONSTANTES (tamaños arrays) ---
...
// Funciones de optimización, búsqueda y ordenamiento
Equipo* buscarEquipo(const char* codigo);
void procesarEstadisticasEquipo(Equipo* eq, int* obtenidas, int* repetidas);
void ordenarRankingDescendente(Equipo** ranking, int* puntajes, int cantidad);
    ...
    for (int i = 0; i < NUM_GRUPOS; i++) {
        for (int j = 0; j < EQUIPOS_POR_GRUPO; j++) {
            if (_stricmp(album[i].equipos[j].codigo, codigo) == 0) {
                return &album[i].equipos[j]; // Retorna puntero al equipo encontrado
            }
        }
    }
...
// Algoritmo de Ordenamiento de Burbuja (Bubble Sort) extraído en función separada
void ordenarRankingDescendente(Equipo** ranking, int* puntajes, int cantidad) {
    for (int i = 0; i < cantidad - 1; i++) {
        for (int j = 0; j < cantidad - i - 1; j++) {
            if (puntajes[j] < puntajes[j + 1]) {
                ...
                // Intercambiar punteros de equipo para mantener sincronía
...
void mostrarRankingEquipos() {
    ...
    // Poblar arrays de ranking y puntajes
    for (int i = 0; i < NUM_GRUPOS; i++) {
        for (int j = 0; j < EQUIPOS_POR_GRUPO; j++) {
    ...
    // Llamada a la función de ordenamiento separada
    ordenarRankingDescendente(ranking, puntajes, idx);
